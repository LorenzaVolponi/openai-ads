...
import { ArrowLeft, ArrowRight, ArrowUpRight, Building2, FileText, Globe2, Handshake, Sparkles } from "lucide-react";

import { GrowthLink } from "@/components/growth-link";
import { LORENZA_ENTITY_ID, LORENZA_GITHUB, LORENZA_LINKEDIN } from "@/lib/lorenza-authority";
...
export default function WorkWithLorenzaPage() {
  const person = {
    ...
    knowsAbout: ["AI systems", "GEO", "AI discovery", "ChatGPT Ads", "conversational advertising", "evidence architecture"],
  };
  ...
  return (
    <main lang="en" className="min-h-screen bg-[#fafaf8] text-zinc-950">
      ...
      <section className="border-b border-zinc-200 bg-white"><div className="mx-auto max-w-7xl px-4 py-16 md:px-6 md:py-24"><div className="inline-flex items-center gap-2 rounded-full border border-zinc-200 px-3 py-1.5 font-mono text-[10px] font-black uppercase tracking-[0.16em] text-zinc-600"><Globe2 className="h-3.5 w-3.5" /> Work together</div><h1 className="mt-6 max-w-6xl font-serif text-[clamp(3.6rem,9vw,8rem)] leading-[0.88] tracking-[-0.06em]">Bring me the problem that is expensive to misunderstand.</h1><p className="mt-7 max-w-3xl text-lg leading-8 text-zinc-600 md:text-xl">I work where AI systems, GEO, discovery, conversational advertising, evidence and business strategy collide. The goal is not another deck. It is a better decision and an executable architecture.</p><div className="mt-8 flex flex-wrap gap-3"><GrowthLink href="/work-with-lorenza/brief" intent="consulting" placement="work-hero-brief" className="inline-flex min-h-12 items-center gap-2 rounded-full bg-zinc-950 px-5 text-sm font-black text-white"><FileText className="h-4 w-4" /> Send a structured brief</GrowthLink><GrowthLink href={LORENZA_LINKEDIN} intent="consulting" placement="work-hero-linkedin" className="inline-flex min-h-12 items-center gap-2 rounded-full border border-zinc-300 bg-white px-5 text-sm font-black">Message Lorenza <ArrowUpRight className="h-4 w-4" /></GrowthLink><GrowthLink href="https://volponi.tech/?utm_source=openai-ads&utm_medium=organic&utm_campaign=work-with-lorenza" intent="consulting" placement="work-hero-volponi" className="inline-flex min-h-12 items-center gap-2 rounded-full border border-zinc-300 bg-white px-5 text-sm font-black">See the wider ecosystem <ArrowRight className="h-4 w-4" /></GrowthLink></div></div></section>
      ...
      <section className="bg-zinc-950 text-white"><div className="mx-auto max-w-7xl px-4 py-16 text-center md:px-6 md:py-24"><p className="font-mono text-[10px] font-black uppercase tracking-[0.16em] text-zinc-400">No funnel theater</p><h2 className="mx-auto mt-4 max-w-5xl font-serif text-5xl leading-[0.95] tracking-[-0.045em] md:text-7xl">If there is a serious opportunity, say what it is.</h2><p className="mx-auto mt-6 max-w-2xl text-base leading-7 text-zinc-400">No hidden lead magnet. No fake urgency. Use the public work to decide whether the thinking is useful — then bring the opportunity directly.</p><div className="mt-8 flex flex-wrap justify-center gap-3"><GrowthLink href="/work-with-lorenza/brief" intent="consulting" placement="work-bottom-brief" className="inline-flex min-h-12 items-center gap-2 rounded-full bg-white px-6 text-sm font-black text-zinc-950"><FileText className="h-4 w-4" /> Open the opportunity brief</GrowthLink><GrowthLink href={LORENZA_LINKEDIN} intent="consulting" placement="work-bottom-linkedin" className="inline-flex min-h-12 items-center gap-2 rounded-full border border-zinc-700 px-6 text-sm font-black text-white">Message Lorenza on LinkedIn <ArrowUpRight className="h-4 w-4" /></GrowthLink></div></div></section>
    </main>
  );
}
