...
export async function GET(
  ...
  const generic = SOCIAL_CARDS[cardKey];

  const size = radar ? RADAR_VARIANTS[radar.variant] : SOCIAL_SIZE;
  const square = radar?.variant === "1x1";
  const tall = square || radar?.variant === "4x3";
  const radarPadding = square ? "68px" : tall ? "62px 70px" : "56px 70px";
  const radarTitleSize = square ? 58 : tall ? 61 : 64;
  const radarSubtitleSize = square ? 23 : 22;
  ...
  const element = radar ? (
    ...
    >
      ...
      <div style={{ display: "flex", flexDirection: "column", maxWidth: 1030 }}>
        <div style={{ fontSize: 17, letterSpacing: 2.6, color: "#52525b", marginBottom: 22 }}>{generic.eyebrow}</div>
        <div style={{ fontSize: 68, lineHeight: 1.02, letterSpacing: -3.4, fontWeight: 800 }}>{generic.title}</div>
        <div style={{ marginTop: 28, fontSize: 25, lineHeight: 1.35, color: "#52525b", maxWidth: 960 }}>{generic.subtitle}</div>
      </div>
