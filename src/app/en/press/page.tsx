...
const citation = `${VOLPONI_AI_INDEX_NAME}, Lorenza Volponi / volponi.tech, edition ${VOLPONI_AI_INDEX_EDITION}.`;
    ...
    <main lang="en" className="min-h-screen bg-[#fafaf8] text-zinc-950">
      ...
      <section className="mx-auto max-w-7xl px-4 py-14 md:px-6 md:py-20">
        <div className="rounded-[2rem] border border-zinc-200 bg-zinc-950 p-8 text-white md:p-10">
          <p className="font-mono text-[10px] font-black uppercase tracking-[0.18em] text-zinc-400">Flagship research · edition {VOLPONI_AI_INDEX_EDITION}</p>
          <h2 className="mt-4 max-w-5xl font-serif text-4xl leading-[0.95] tracking-[-0.04em] md:text-6xl">{VOLPONI_AI_INDEX_NAME}</h2>
      ...
      <section className="border-y border-zinc-200 bg-[#f4f4f1]">
        <div className="mx-auto max-w-7xl px-4 py-14 md:px-6 md:py-20">
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-2">{indexDimensions.map((dimension) => <article key={dimension.id} className="rounded-3xl border border-zinc-200 bg-white p-6"><h2 className="text-xl font-black tracking-[-0.025em]">{dimension.label}: {dimension.state}</h2><p className="mt-3 text-sm leading-7 text-zinc-600">{dimension.finding}</p><p className="mt-2 text-xs leading-5 text-zinc-500">{dimension.interpretation}</p><a href={dimension.source.url} target="_blank" rel="noopener noreferrer" className="mt-4 inline-flex items-center gap-1 text-xs font-black">Primary source <ArrowUpRight className="h-3.5 w-3.5" /></a></article>)}</div>
          <div className="mt-6 rounded-3xl border border-zinc-200 bg-white p-6"><p className="font-mono text-[10px] font-black uppercase tracking-[0.16em] text-zinc-500">Quick facts</p><ul className="mt-4 space-y-2 text-sm leading-6 text-zinc-700">{pressFindings.map((finding) => <li key={finding}>— {finding}</li>)}</ul></div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-4 py-14 md:px-6 md:py-20">
        <div className="grid gap-5 lg:grid-cols-3">
          ...
          <article className="rounded-3xl border border-zinc-200 bg-white p-7"><Quote className="h-5 w-5" /><h2 className="mt-4 text-2xl font-black">Citation</h2><p className="mt-3 text-sm leading-7 text-zinc-600">{citation}</p><Link href="/citation.json" className="mt-4 inline-flex text-xs font-black hover:underline">Citation guide</Link></article>
