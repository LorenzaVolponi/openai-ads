...
export function AuthorShowcase() {
  return (
    <section id="autoria" className="content-auto relative overflow-hidden border-y border-zinc-200 bg-white text-zinc-950">
      <div className="pointer-events-none absolute -right-24 -top-24 h-96 w-96 rounded-full border-[48px] border-zinc-950/[0.035]" />
      <div className="pointer-events-none absolute -bottom-32 -left-28 h-[28rem] w-[28rem] rounded-full border-[72px] border-zinc-950/[0.035]" />
      ...
      <div className="relative mx-auto grid max-w-7xl gap-12 px-4 py-16 md:px-6 md:py-24 lg:grid-cols-[1.15fr_0.85fr] lg:items-end">
        <div>
          ...
          <div className="mt-8 grid gap-3 sm:grid-cols-3">
            <div className="rounded-2xl border border-zinc-200 bg-[#fafaf8] p-5">
              <Braces className="h-5 w-5" />
              <p className="mt-4 text-sm font-black uppercase tracking-[0.12em]">Builder</p>
              <p className="mt-2 text-sm leading-6 text-zinc-600">Código, produto, automação e sistemas como linguagem de execução.</p>
            </div>
            <div className="rounded-2xl border border-zinc-200 bg-[#fafaf8] p-5">
              <RadioTower className="h-5 w-5" />
              <p className="mt-4 text-sm font-black uppercase tracking-[0.12em]">AI systems</p>
              <p className="mt-2 text-sm leading-6 text-zinc-600">Busca, GEO, agentes, evidência digital e arquitetura de informação.</p>
            </div>
            <div className="rounded-2xl border border-zinc-200 bg-[#fafaf8] p-5">
              <Sparkles className="h-5 w-5" />
              <p className="mt-4 text-sm font-black uppercase tracking-[0.12em]">Estratégia</p>
              <p className="mt-2 text-sm leading-6 text-zinc-600">Conectar negócio, narrativa e tecnologia sem separar pensar de construir.</p>
            </div>
          </div>
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            ...
            <Button asChild variant="outline" className="border-zinc-300 bg-white text-zinc-950 hover:bg-zinc-950 hover:text-white">
        ...
        <aside className="rounded-3xl border border-zinc-950 bg-zinc-950 p-6 text-white shadow-2xl md:p-8">
          <div className="flex items-center justify-between gap-4">
            <div>
              <p className="font-mono text-[11px] uppercase tracking-[0.18em] text-zinc-400">Press / AI citation block</p>
              <h3 className="mt-2 text-2xl font-black tracking-tight">Pode citar assim.</h3>
            </div>
            <Quote className="h-7 w-7 text-zinc-700" />
          </div>

          <blockquote className="mt-7 border-l-2 border-white/40 pl-5 text-sm leading-7 text-zinc-300">
