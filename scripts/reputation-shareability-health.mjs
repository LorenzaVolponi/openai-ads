...
const routes = ["/reputation-signals.json", "/shareable-quotes.json", "/journalist-mode.json", "/distribution-manifest.json", "/person.json", "/citation.json"];
const failures = [];
...
for (const route of routes) {
  try {
    ...
    if (route === "/reputation-signals.json") {
      ...
      if (!/externally verifiable/i.test(JSON.stringify(parsed.boundary))) failures.push("reputation-signals.json: evidence boundary missing");
    }
    if (route === "/shareable-quotes.json") {
      const parsed = JSON.parse(body);
      if (!parsed.quotes?.length) failures.push("shareable-quotes.json: quote objects missing");
      if (!parsed.quotes?.every((q) => q.canonical && q.social?.linkedin && q.social?.x && q.social?.instagram)) failures.push("shareable-quotes.json: incomplete social object");
    }
